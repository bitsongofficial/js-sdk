...
describe("crypto", () => {
  ...
  it("generate an address from privateKey", () => {
    ...
    const address = crypto.getAddressFromPrivateKey(privateKey)
    expect(address).toBe("bitsong1hgm0p7khfk85zpz5v0j8wnej3a90w709z6mf2a")
  })
  ...
  it("generate private key from mnemonic", () => {
    ...
    expect(pk.toString("hex")).toBe(
      "453268f9acf09be41db28a218d660ef169cea795ec3607b4ffbd4b8483c9674e"
    )
  })
  ...
  it("generate address from mnemonic", () => {
    ...
    expect(address).toBe("bitsong18j94qw97ag42lg6uany20gma2y3uwfluwgg2jd")
  })
  ...
  it("generateSignature and verifySignature - utf8 memo", () => {
    ...
    const msg = Buffer.from(
      '{"account_number":1,"data":"ABCD","chain_id":"bitsong-network","memo":"music for everybody!☺","msgs":["msg"],"sequence":1,"source":1}'
    ).toString("hex")
    const sig = crypto.generateSignature(msg, privateKey).toString("hex")
    expect(crypto.verifySignature(sig, msg, publicKey)).toBeTruthy()
  })
